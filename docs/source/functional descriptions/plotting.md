...
Continuous S_pMSE values are mapped to discrete intervals that represent meaningful divergence levels. Each interval is associated with a fixed colour, enabling immediate visual differentiation between low, moderate and high values. The S_pMSE values are visualised in five bins: ${\text(0,3]}$, ${\text(3,10]}$, ${\text(10,30]}$, ${\text(30,100]}$, ${\text(100,\infty)}$. Missing values and a S_pMSE of 0 get their own distinguishable bin. The colour scheme must be sequential, colour-blind friendly, print-friendly, and maintain contrast when converted to grayscale. Examples are discrete colours taken from the continous colour schemes of python package `plotly`: $\text{YlOrBr}$, $\text{Reds}$. Or python package `tol_colors`: $\text{iridescent}$, $\text{YlOrBr}$.
