...
Both approaches use the same synthesis method. Explicitly passing `CartMethod()` is therefore only required when you want to make the choice of synthesis method explicit or customise its configuration. See {ref}`Guide 3.1.4: Configuring CART <314-configuring-cart>` and the [Example: Configure CART directly](../examples/configure_cart_directly.md) for more information on the latter topic.
...
For a target column $Y$, CART learns an approximation of its conditional distribution, given previously synthesised columns:
```{math}
...
Here, `X` contains the predictor columns that have previously been synthesised, while `y` is the observed target column that the method learns to synthesise. After fitting, `transform(X)` generates values for that target column, based on the supplied predictors.
