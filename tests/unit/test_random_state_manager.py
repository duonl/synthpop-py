...
def test_random_state_manager_given_uninitialised_when_creating_rng(monkeypatch, patch_default_rng):
    ...
    # We patch secrets.randbits so that we can control the returned value
    # and assert that the correct seed is used to create the RNG.
    def mock_random_bits(n):
        assert n == 128, "generated seeds should be 128 bits"
        return 1234
    monkeypatch.setattr(secrets, "randbits", mock_random_bits)
    ...
    assert result is patch_default_rng["expected_rng"], (
        "the returned RNG should be from np.random.default_rng"
    )
...
def test_random_state_manager_given_uninitialised_when_creating_instance_seed(monkeypatch, mocker):
    ...
    mock_create_seed_from_sequence = mocker.patch(
        "synthpop.reproducibility._create_seed_from_sequence",
        return_value=expected_returned_seed
    )
...
def test_random_state_manager_initialised_exit(monkeypatch):
    ...
    Given: 
        ...
        - a context block with a RandomStateManager instance has been entered (__enter__ has been called)
    ...
    # That means that the old_seed is set to the initial seed, and
    # old_seed_sequence is set to the initial seed sequence.
...
def test_random_state_manager_raises_on_invalid_type():
    with pytest.raises(
        TypeError,
        match=re.escape("expects int or sequence of ints")
    ):
    ...
    with pytest.raises(
        ValueError,
        match=re.escape("expected non-negative integer")
    ):
...
def test_random_state_manager_warns_on_empty_list():
    ...
    assert len(record) == 1
    assert str(
        record[0].message) == "empty list as no entropy for seed. Use None for system entropy."
