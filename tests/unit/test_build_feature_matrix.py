...
from synthpop.methods.tree_utils import _build_feature_matrix
...
def test_build_feature_matrix_empty_input_empty_output():
    X = {}
    result = _build_feature_matrix(X, [])
    assert result.shape == (0, 0)


def test_build_feature_matrix_single_column():
    X = {"a": np.array([1, 2])}
    result = _build_feature_matrix(X, ["a"])
...
def test_build_feature_matrix_two_columns():
    ...
    result = _build_feature_matrix(X, ["a", "b"])
...
def test_build_feature_matrix_respects_order():
    ...
    result = _build_feature_matrix(X, ["b", "a"])
