...
from synthpop.methods.tree_utils import (
    _sample_array,
    _fit_decision_tree_with_reachable_leaves,
    _check_all_leaf_nodes_are_reached
)
...
def test_fit_decision_tree_with_reachable_leaves_retry_when_unreachable(mocker):
    ...
    for i in range(len(expected_random_states)):
        ...
        assert tree.random_state_ == expected_random_states[i], (
            "The random_state has not been set"
        )
        ...
        assert np.array_equal(tree.fit_X, X), (
            "tree has not been fitted again with correct X before passing it to _check_all_leaf_nodes_are_reached"
        )
        assert np.array_equal(tree.fit_y, y), (
            "tree has not been fitted again with correct y  before passing it to _check_all_leaf_nodes_are_reached"
        )
...
def test_sample_array_maps_rng_output_to_sampled_values():
    ...
    # low is 0, high is 4, n_samples is 4
    assert rng.called_with == [(0, 4, 4)]
