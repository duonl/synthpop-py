...
def get_post_synth_test_data():
    ...
    y_values_np_array = [(np.array(["a",missing,"b", missing], dtype=str_dtype), np.array(["a",np.nan,"b", np.nan], dtype=str_dtype), missing) for missing in missing_markers] + \
    [(np.array(["a","c","b",missing], dtype=str_dtype), np.array(["a","c","b",np.nan], dtype=str_dtype), missing) for missing in missing_markers] + \
    [(np.array(["a","c","b",not_missing], dtype=str_dtype), np.array(["a","c","b",not_missing], dtype=str_dtype), None) for not_missing in ["not missing","missing","Nan","None"]] 
...
def test_clone_works_and_fitted_does_not_preserve_state():
    rpnwv = ReplaceMissingWithValue(missing_marker="N.a.N.")
    ...
    assert hasattr(rpnwv, "missing_marker")
