...
def get_test_data_full():
    ...
    input_2D = [
        (X.reshape((-1, 1)), y, expected_input_pca, pca_result, expected_dict)
        for pca_result, expected_dict in get_pca_return_and_dict()
    ]
    input_with_lists = [
        (X.tolist(), y.tolist(), expected_input_pca, pca_result, expected_dict)
        for pca_result, expected_dict in get_pca_return_and_dict()
    ]
...
def get_test_data_target_constant_missing():
    return [
        (
            ...
            np.array([np.nan] * 5, dtype=str_dtype),
            None,
            None,
...
def get_test_data_missing_target():
    ...
    regular_target = [
        (
            X,
            np.array(["x", np.nan, "y", "z", np.nan, np.nan], dtype=str_dtype),
            expected_input_pca,
            pca_result,
            expected_dict | {"c": [np.nan] * len(expected_dict["a"])},
        )
        for pca_result, expected_dict in get_pca_return_and_dict()
    ]
    ...
    small_target = [
        (
            np.array(["a", "a", "b"], dtype=str_dtype),
...
def validate_mapping(result_mapping, pca_result, expected_keys):
    for (i, key) in enumerate(expected_keys):
        assert np.allclose(
            ...
            f"invalid mapping for {key}. "
            f"Expected {pca_result[i]}, actual: {result_mapping[key]}"
        )
...
def test_pca_fit_numeric_correctness(
    ...
    if expected_input_for_PCA is not None:
        assert np.allclose(
            result.pca_transform_.transform_X_,
            expected_input_for_PCA,
        ), "input for PCA not calculated correctly"
...
def test_pca_fit_empty_input():
    ...
    since the expected behaviour for empty datasets is currently undefined in the functional description.
    """
    encoder = PCAEncoder(pca_transform=None)
        ...
        (
            ...
            np.array(  # Expected output
                [
                    [1.2, 3.4],  # a
                    [1.2, 3.4],  # a
                    # b, note that numpy turns these Nones to nans
                    # when casting to float32
                    [np.nan, np.nan],
                    [np.nan, np.nan],  # b
                    [5.6, 7.8],  # c
                    [np.nan, np.nan],
...
@pytest.mark.parametrize(
    "X, mapping, expected_output",
    get_test_data_transform(),
)
...
def test_pca_transform_exception_on_new_value():
    """
    An informative exception should be raised when the user attempts
    to encode a value that was not in the data during fitting.
    """
    encoder = PCAEncoder(pca_transform=None)
