...
class StubTreeMethod(_AbstractTreeMethod):
    def __init__(
...
def tree_method(encoder, missing_handling, leafnode_sampler):
    return StubTreeMethod(
        encoder=encoder,
        missing_handling=missing_handling,
        tree_sampler=leafnode_sampler,
        tree=StubTree(apply_result=[1, 2, 3])
    )
...
def assert_dict_array_equal(expected, actual):
    ...
    assert len(expected.keys()) == len(actual.keys()), (
        f"actual has more keys than expected. "
        f"Expected: {len(expected.keys())}. Actual: {actual.keys()}"
    )
...
def test_fit_calls_rare_category_check(X, y, index_cat, threshold, tree_method):
    ...
    for cat_col in index_cat:
        ...
        calls = synthpop.utils._warn_on_rare_category.call_args_list
    ...
    assert len(index_cat) == synthpop.utils._warn_on_rare_category.call_count
...
def test_fit_raises_on_rare_category_not_called_when_disabled(
    ...
    synthpop.utils._warn_on_rare_category.assert_not_called()
...
def test_fit_trains_encoder(X, y, index_cat, tree_method):
    ...
    for i in index_cat:
        ...
        for i2 in index_cat:
            assert (
                tree_method.encoders_[i] is not tree_method.encoders_[i2]
            ) or i2 == i
...
def test_fit_prepare_data_for_fit_is_called(X, y, index_cat, tree_method):
    ...
    assert tree_method.missing_handler_ is not tree_method.missing_handler
...
def test_fit_build_feature_matrix(X, y, index_cat, tree_method, mocker):
    ...
    X_exp = {
        k: (
            tree_method.encoders_[k].transform_return_value
            if k in index_cat
            else v
        )
...
def test_fit_tree_is_fit(X, y, index_cat, tree_method, mocker):
    ...
    expected_y = tree_method._convert_y(
        tree_method.missing_handler_.prepared_for_fit_result[1])

    mock_fit_decision_tree_with_reachable_leaves.assert_called_once()
...
def test_fit_classifier_converts_to_str(encoder, leafnode_sampler, mocker):
    ...
    mocked_to_str = mocker.patch(
        "synthpop.methods.cart_synth._to_fixed_length_string_array",
        return_value=str_y,
    )
