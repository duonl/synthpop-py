...
def get_test_data_classifier(seed=10, n_samples=100, with_cats=False, with_missing_features=False, with_missing_target=False):
    X, y = make_classification(
        random_state=seed, n_samples=n_samples, n_classes=10, n_informative=11)
    ...
    if with_cats:
        for idx in idx_cats:
            ...
            x_i = [f % 5 for f in x]
            X[idx] = np.array([string.ascii_lowercase[i % 26]
    ...
    if with_missing_target:
        y = np.array([string.ascii_lowercase[i % 26] if i %
                     5 != 0 else np.nan for i in y], dtype=str_dtype)
    else:
        y = np.array([string.ascii_lowercase[i % 26]
                     for i in y], dtype=str_dtype)
    return (X, y)
