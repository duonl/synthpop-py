...
def make_data_missing(X, as_series=False):
    ...
    for ik, k in enumerate(X.keys()):
        # The missingness is periodic. Every p-th element is missing.
        ...
        if isinstance(X[k].dtype, pd.CategoricalDtype):
            ...
            # for internal TreeMethod datatypes
            elif pd.api.types.is_numeric_dtype(X[k].dtype):
                X[k] = np.array(values)
...
def get_test_data_classifier(
    ...
    n_samples=100,
    with_cats=False,
    with_missing_features=False,
    with_missing_target=False,
):
    ...
    if with_cats:
        for idx in idx_cats:
            ...
            x_i = [f % 5 for f in x]
            X[idx] = np.array(
                [string.ascii_lowercase[i % 26] for i in x_i],
                dtype=str_dtype,
            )
    ...
    if with_missing_target:
        y = np.array(
            [
                string.ascii_lowercase[i % 26] if i % 5 != 0 else np.nan
                for i in y
            ],
            dtype=str_dtype,
        )
        ...
        y = np.array(
            [string.ascii_lowercase[i % 26] for i in y],
            dtype=str_dtype)
    return X, y
...
def get_test_data_regressor(
    ...
    if with_cats:
        for idx in idx_cats:
            x = (X[idx] * 10).astype(int)
            x_i = [f % 26 for f in x]
            X[idx] = np.array([string.ascii_lowercase[i]
                              for i in x_i], dtype=str_dtype)
...
def simulate_realistic_dataset_correlations(n_samples=100):
    ...
    fourth_column = [
        first_column[i] if third_column[i] in ["a", "b"]
        else second_column[i]
        for i in range(n_samples)
    ] + rng.random((n_samples,)) * 0.1

    # fifth column is categorial with many levels and
    # correlated with both numeric and categorical columns
    # This is done by calculating a numeric value roughly between 0 and 26
    # and map that value to the alphabet.

    # The third column decides if the fifth is near the begin or the end of the alphabet.
    distribution_general_means = [
        9 if third_column[i] in ["b", "c"]
        else 18 for i in range(n_samples)
    ]
    ...
    alphabet_index = [
        int(rng.normal(distribution_means[i], 6)) % 26
        for i in range(n_samples)
    ]

    fifth_column = [
        string.ascii_lowercase[alphabet_index[i]]
        for i in range(n_samples)
    ]
