...
CLASSIFIER_CASES = [
    ...
    (rigged_tree_classifier_method(), *
     get_test_data_classifier(with_cats=True, with_missing_features=True)),
    (rigged_tree_classifier_method(), *get_test_data_classifier(with_cats=True,
     with_missing_features=True, with_missing_target=True)),
...
def test_order_of_input_dict_does_not_change_output(method, X, y):
    ...
    assert np.array_equal(feature_matrix_fit,
                          feature_matrix_apply, equal_nan=True)

    X_different_order = {k: X[k] for k in sorted(X.keys(), reverse=True)}

    assert list(X_different_order.keys()) != list(
        X.keys()), "test invalid, order of features has not been changed"
    ...
    assert np.array_equal(feature_matrix_fit,
                          feature_matrix_different_order, equal_nan=True)
...
def multiset_frequency_structure_matches(a, b):
    ...
    hist_a = np.sort(
        np.unique(a[a_not_nan], return_counts=True, equal_nan=True)[1])
    hist_b = np.sort(
        np.unique(b[b_not_nan], return_counts=True, equal_nan=True)[1])
...
def test_fit_preserves_feature_value_frequencies(method, X, y):
    ...
    for (i, k) in enumerate(X.keys()):
        assert multiset_frequency_structure_matches(
            X[k], method.tree_.fit_X[:, i]), f"histogram mismatch on key {k}"
...
def test_classifier_missing_target(method, X, y):
    y = np.array([v if i % 3 == 0 else np.nan for (
        i, v) in enumerate(y)], dtype=str_dtype)
...
def test_regressor_trainings_data_reaches_all_nodes():
    ...
    X,y = get_test_data_regressor(n_samples=50,seed=80,with_cats=True,with_missing_features= True,with_missing_target=True)
...
def test_regression_bug_129_classifier_no_empty_leaf_failure():
    tree_seed,data_seed = 91,80 #These seeds reproduces #129 for the classifier
    method = TreeClassifierMethod(
            tree=SpyDecisionTreeClassifier(
                min_samples_leaf=5,    # equivalent to minbucket in synthpop-r
                min_impurity_decrease=1e-08,   # equivalent to cp in synthpop-r
